...
def seed_data():
    with app.app_context():
        ...
        print(f"  {db.session.execute(db.select(db.func.count(User.id))).scalar()} usuários")
        print(f"  {db.session.execute(db.select(db.func.count(Category.id))).scalar()} categorias")
        print(f"  {db.session.execute(db.select(db.func.count(Task.id))).scalar()} tasks")
